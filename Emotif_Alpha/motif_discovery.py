...
"""
Running motif discovery in parallel and combining the result to
meme format PWM.

Author: Yichao Li
Last modification: 5-27-2019


input
-----

required: foreground fasta
required: background fasta
...
a PWM file combining motif discovery results from multiple tools
...
def main():
	...
	main_parser = argparse.ArgumentParser(description="motif discovery",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
	main_parser.add_argument('-f',"--foreground",  help="input foreground fasta sequence", required=True)
	main_parser.add_argument('-b',"--background",  help="input background fasta sequence", required=True)
	...
	extend_gRNA_sequence(args,editing_base,editing_window)
